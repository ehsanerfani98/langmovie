...
@section('actions')
    <a href="{{ route('video.create') }}" class="btn btn-success btn-sm btn-icon-split">
                ...
                <tbody>
                    @foreach ($videos as $video)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('video.edit', $video->id) }}" class="btn btn-primary btn-sm">ویرایش</a>
                                <form action="{{ route('video.destroy', $video->id) }}" method="POST" class="d-inline" onsubmit="return confirm('آیا مطمئن هستید؟')">
                                    @csrf
                                    @method('DELETE')
